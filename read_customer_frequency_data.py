...
@st.cache_data(ttl=600, show_spinner="Preparando dados para download...")
def read_all_customer_data(
    sales_channel=None, 
    name=None, 
    phone_number=None, 
    ...
    """
    Busca TODOS os dados de frequência de compra de clientes (sem paginação) para download.
    """
    ...
    TRANSACOES AS (
      SELECT CU.ID, DATE(OT.CREATED_AT) AS SALES_DATE, OT.TOTAL_BAG_DETAIL AS TOTAL, 'iFood' AS SALE_CHANNEL
      FROM ORDERS_TABLE OT
      INNER JOIN CUSTOMER_UNION CU ON CU.CUSTOMER_ID_ORIGINAL = OT.CUSTOMER_ID
      UNION ALL
      SELECT CU.ID, DATE(OT.CREATED_AT) AS SALES_DATE, OT.TOTAL_BAG_DETAIL AS TOTAL, '99food' AS SALE_CHANNEL
      FROM ORDERS_TABLE OT
      INNER JOIN CUSTOMER_UNION CU ON CU.C_UID_ORIGINAL = OT.C_UID
      UNION ALL
      SELECT CU.ID, DATE(SC.CREATED_AT) AS SALES_DATE, SC.TOTAL AS TOTAL, 'Loja' AS SALE_CHANNEL
      FROM SALES_CLUB SC
      INNER JOIN CUSTOMER_UNION CU ON CU.DOCUMENT_NUMBER_CTB = SC.CLIENT_CPF
    ),
    COMPRAS AS (
      SELECT ID, SALES_DATE, TOTAL, SALE_CHANNEL,
        EXTRACT(DAY FROM (DATE(SALES_DATE) - LAG(DATE(SALES_DATE)) OVER (PARTITION BY ID ORDER BY SALES_DATE))) AS dias_entre_compras
      FROM TRANSACOES
    ),
    RESUMO AS (
      SELECT
        ID, COUNT(1) AS QTD_COMPRAS, SUM(TOTAL) AS TOTAL_GASTO,
        MIN(SALES_DATE) AS PRIMEIRA, MAX(SALES_DATE) AS ULTIMA,
    ...
    try:
        ...
        st.error(f"Erro ao buscar dados para download: {e}")
        return pd.DataFrame()
